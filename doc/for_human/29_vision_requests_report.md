...
## 상세 (vision.md 추가요청 #134~#154)

| # | 요청 요약 | 상태 | 처리 방법 |
|---|-----------|------|-----------|
| 134 | 로그인 버튼 border-radius | ✅ | `LandingPage` `landing-login-submit` `rounded-lg` ([`260525-11`](../for_agent/plans/260525-11.md)) |
| 135 | 최신 링크 배너 디자인 | ✅ | 한 줄 링크 텍스트 · `deliverableLinks.ts` ([`260525-11`](../for_agent/plans/260525-11.md)) |
| 136 | 팀명 저장 안 됨 | ✅ | `updateTeamProfileInDb` 존재하지 않는 `updated_at` 제거 · 헤더 focus 갱신 ([`260525-11`](../for_agent/plans/260525-11.md)) |
| 137 | 공지 클릭→상세 페이지 | ✅ | `CourseAnnouncementDetailPage` · `/announcements/:announcementId` ([`260525-11`](../for_agent/plans/260525-11.md)) |
| 138 | 랜덤 팀 4·3명 버튼 제거 | ✅ | `StudentsNetworkPage` 숫자 입력만 ([`260525-11`](../for_agent/plans/260525-11.md)) |
| 139 | node_modules 제외 무압축 업로드 | ✅ | `projectSourceZip` `compression: STORE` · 안내 문구 ([`260525-11`](../for_agent/plans/260525-11.md)) |
| 140 | 챗리스트·1:1 목록 페이지 | ✅ | `/courses/:id/messages` · `DirectChatPanel` · 수강자 배너 「챗리스트」([`260525-12`](../for_agent/plans/260525-12.md)) |
| 141 | 교수 프로필카드 수강자들 | ✅ | `courseProfessor` 배너 · 학생·교수 공통 노출 ([`260525-12`](../for_agent/plans/260525-12.md)) |
| 142 | 기존 수업 팀플 스테이지 수정 | ✅ | `CourseStagesEditPage` · `api.teamStages.replace` ([`260525-12`](../for_agent/plans/260525-12.md)) |
| 143 | DB 반영 느림 | ✅ | MCP team/course Realtime publication · `useDebouncedRealtimeReload` · Teams/Courses/Students ([`260526-1`](../for_agent/plans/260526-1.md)) |
| 144 | 교수 프로필 확장 | ✅ | `ProfessorProfilePage` bio · `saveProfessorProfile` ([`260525-12`](../for_agent/plans/260525-12.md)) |
| 145 | 링크 클릭 시 다운로드 | ✅ | `externalDeliverableHref` · 링크 `download` 미사용 ([`260525-11`](../for_agent/plans/260525-11.md)) |
| 146 | 탈퇴·재가입 팀 목록 갱신 | ✅ | `TeamsPage` `window` focus 시 `reloadTeams` ([`260525-11`](../for_agent/plans/260525-11.md)) |
| 147 | 조원평가 키워드·커스텀 | ✅ | `DEFAULT_PEER_REVIEW_GOOD_KEYWORDS` · 커스텀 추가 · `TeamPeerReviewPage` ([`260525-12`](../for_agent/plans/260525-12.md)) |
| 148 | 교수 기여도 조회 | ✅ | `contribution_rating` MCP · 조원평가 입력·교수 조회 UI ([`260525-12`](../for_agent/plans/260525-12.md)) |
| 149 | 수업 삭제 후 잔존 | ✅ | `deleteCourseInDb` direct_messages·course_materials 삭제 ([`260525-11`](../for_agent/plans/260525-11.md)) |
| 150 | 리포트 팀카드→모달 | ✅ | `MyPage` `selectedReportTeam` · `mypage-report-team-detail-modal` ([`260525-11`](../for_agent/plans/260525-11.md)) |
| 151 | 반응형 깨짐 | ✅ | `MainLayout`·본문 `overflow-x-clip`·`min-w-0` · 팀 그리드 ([`260526-1`](../for_agent/plans/260526-1.md)) |
| 153 | 로그인 버튼 세로 패딩 | ✅ | `landing-login-submit` `py-1.5` `min-h-[2.625rem]` ([`260526-1`](../for_agent/plans/260526-1.md)) |
| 152 | 교수 산출물 업로드 금지 | ✅ | `assertStudentOwnTeamWrite` · `canUploadDeliverable` 학생만 ([`260525-11`](../for_agent/plans/260525-11.md)) |
| 154 | 챗리스트 스레드 클릭 오류 | ✅ | DM peer 검증 · Realtime 채널 중복 구독 제거(`useIsSmUp` 단일 패널) ([`260526-3`](../for_agent/plans/260526-3.md)) |
| 155 | 폴더 업로드 시 화면 멈춤 | ✅ | `AiGeneratingIndicator` 오버레이 · 진행률 · 배치 yield ([`260526-5`](../for_agent/plans/260526-5.md)) |
| 156 | 폴더 업로드 압축 | ✅ | ZIP 제거 · `collectProjectFolderFiles` 개별 업로드 · 경로 유지 ([`260526-5`](../for_agent/plans/260526-5.md) 보완) |
| 157 | 산출물+링크 제출 시 배너 Storage URL | ✅ | `deliverableDeployUrl`·최신 배포 링크만 배너 ([`260526-6`](../for_agent/plans/260526-6.md)) |
| 158 | 미확인 공지·메시지 프로필 점멸 | ✅ | `navInboxSeen`·`NavProfileInbox` 빨간 점 pulse ([`260526-6`](../for_agent/plans/260526-6.md)) |
| 159 | 프로필 hover 미확인 목록 | ✅ | `useNavInbox` 드롭다운·공지/메시지 링크 ([`260526-6`](../for_agent/plans/260526-6.md)) |
| 160 | 수업 생성 시 시작·종료일 | ✅ | `start_date`·`end_date` MCP · 생성 모달·카드·상세 ([`260526-7`](../for_agent/plans/260526-7.md)) |
| 161 | 수업 생성 후 모달 유지 | ✅ | 성공 즉시 `setShowCreateModal(false)` · 중복 제출 방지 ([`260526-8`](../for_agent/plans/260526-8.md)) |
| 162 | 생성 시 스테이지 미반영 | ✅ | `ai_course_stages` insert·글로벌 fallback 제거 ([`260526-8`](../for_agent/plans/260526-8.md)) |
| 163 | 수강자들 교수 카드 그리드 | ✅ | `ProfessorNetworkCard` · 교수 1칸·학생 2칸 ([`260526-8`](../for_agent/plans/260526-8.md)) |

---

## AI 유지 규칙 (에이전트)

1. `vision.md` 변경 시 `27`·본 문서 §상세(1~#163)·`26`을 같은 세션에 갱신한다.
