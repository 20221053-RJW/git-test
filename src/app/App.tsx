          ...
          <div className="flex flex-row flex-1">
            {activeTab === 'classes' && (
              <aside className="w-20 bg-gradient-to-b from-amber-50 to-amber-100 flex flex-col items-center py-6 gap-3 border-r border-amber-200 overflow-y-auto h-full">
                <button
                  onClick={() => changeTab('team')}
                  className={`w-16 h-16 rounded-xl font-bold transition-all duration-200 flex flex-col items-center justify-center gap-1 ${
                    activeTab === 'team'
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
              {classmates.map((c, index) => (
                <div
                  ...
                  className={`bg-white border-2 rounded-xl p-6 flex flex-col items-center text-center cursor-pointer hover:shadow-lg transition min-h-[480px] ${
                    c.name === '류지원 (나)' ? 'border-blue-500 hover:border-blue-600' : 'border-gray-200 hover:border-blue-300'
                  }`}
                >
